...
export default function Navbar({ lang, dict }: NavbarProps) {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-ivory/90 backdrop-blur-sm">
        <Container>
          <nav className="flex h-20 items-center justify-between">
            <Link href={`${prefix}/`} aria-label="Narvaez Digital Marketing — Home">
              <svg
                ...
                aria-hidden="true"
              >
                <rect x="5.79" y="5.88" width="44.02" height="42.69" rx="3.24" ry="3.24" fill="#343434" />
                <path d="M41.62,39.42v-19.83c0-4.09-3.21-7.43-7.25-7.62h-.85c-.06,0-.11,0-.16.01-.98.07-1.9.33-2.74.76-2.29,1.11-3.94,3.34-4.23,5.99v.92h5.6c.76,0,1.37.61,1.37,1.36v18.42c0,1.48-1.2,2.68-2.68,2.71h-.06v.33h13.75v-.3c-1.52,0-2.75-1.23-2.75-2.75Z" fill="#ececed" />
                <path d="M14.09,16.86l-2.75,1.22v1.6h1.41c.74,0,1.34.6,1.34,1.34v18.41c0,1.52-1.23,2.75-2.75,2.75v.3h13.76v-.34h-.04c-1.5-.02-2.71-1.22-2.71-2.71v-18.37c0-.76.62-1.38,1.38-1.38h1.37v-7.7l-2.75,1.22-8.26,3.66Z" fill="#ececed" />
              </svg>
            </Link>
            ...
            <ul className="hidden md:flex items-center gap-10 text-sm list-none m-0 p-0">
              {links.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="link-underline text-graphite hover:text-charcoal transition-colors"
                  >
                    {l.label}
                  </Link>
                </li>
              ...
              <li>
                <Link
                  href={switchHref}
                  className="text-xs uppercase tracking-widest text-graphite/40 hover:text-charcoal transition-colors border border-neutral-light px-3 py-1.5"
                >
        ...
        <div className="flex flex-col justify-center items-start h-full px-8 sm:px-10">
          ...
          <div className="mt-12 sm:mt-16 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
            ...
            <Link
              href={switchHref}
              onClick={() => setIsOpen(false)}
              className="text-xs uppercase tracking-widest text-ivory/40 hover:text-ivory transition-colors border border-ivory/20 px-3 py-1.5 w-fit"
            >
              {switchLang.toUpperCase()}
            </Link>
          </div>
